...
<main class="workspace">
    ...
    <div class="table-scroll"><table class="data-table purchase-wide-table">
        <thead><tr><th>발주</th><th>품목</th><th>협력회사</th><th>발주수량</th><th>입고수량</th><th>검수결과</th><th>출하일시</th><th>입고일시</th><th>상태</th><th>마감</th></tr></thead>
        <tbody><c:forEach items="${receivings}" var="row"><tr>
            ...
            <td><fmt:formatNumber value="${row.receivedQty}" /> ${row.unit}</td><td><c:out value="${row.inspectionResult}" default="검수 대기" /></td>
            <td><c:out value="${row.shippedAt}" default="-" /></td><td><c:out value="${row.receivedAt}" default="-" /></td>
        ...
        </tr></c:forEach><c:if test="${empty receivings}"><tr><td colspan="10" class="empty-cell">출하·입고 연계 건이 없습니다.</td></tr></c:if></tbody>
    </table></div></section>
