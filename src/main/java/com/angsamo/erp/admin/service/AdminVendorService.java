...
public class AdminVendorService {
	...
	public void create(VendorItem vendor) {
		normalize(vendor);
		if (mapper.countByCode(vendor.getVendorCode()) > 0) throw new IllegalArgumentException("\uC774\uBBF8 \uC0AC\uC6A9 \uC911\uC778 \uD611\uB825\uD68C\uC0AC \uCF54\uB4DC\uC785\uB2C8\uB2E4.");
	...
	private void normalize(VendorItem vendor) {
		String code = vendor.getVendorCode() == null ? "" : vendor.getVendorCode().trim().toUpperCase();
		String name = vendor.getVendorName() == null ? "" : vendor.getVendorName().trim();
		if (!code.matches("[A-Z0-9_-]+")) throw new IllegalArgumentException("\uD611\uB825\uD68C\uC0AC \uCF54\uB4DC\uB294 \uC601\uBB38, \uC22B\uC790, _, -\uB9CC \uC0AC\uC6A9\uD558\uC138\uC694.");
		if (name.isBlank()) throw new IllegalArgumentException("\uD611\uB825\uD68C\uC0AC\uBA85\uC744 \uC785\uB825\uD558\uC138\uC694.");
		vendor.setVendorCode(code); vendor.setVendorName(name);
	}
}
